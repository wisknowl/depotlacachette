<div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; margin-bottom: 20px;">
    <div class="page-title" style="margin-bottom: 0;">
        ...
        <h1>Achats & Approvisionnements</h1>
    </div>
...
<div class="card" style="margin-bottom: 20px; padding: 16px 20px; background: white;">
    ...
    <div style="display: flex; gap: 10px; margin-bottom: 15px; padding-bottom: 12px; border-bottom: 1px solid #F1F5F9; align-items: center; flex-wrap: wrap;">
        ...
        <a href="<?= BASE_URL ?>/achats?payment_mode=all<?= !empty($filterSearch) ? '&search='.urlencode($filterSearch) : '' ?><?= !empty($filterSupplierId) ? '&supplier_id='.$filterSupplierId : '' ?>" 
           ...
           style="padding: 5px 12px; font-size: 0.82rem; <?= ($filterPaymentMode !== 'all' && !empty($filterPaymentMode)) ? 'background: #F1F5F9; color: #334155;' : '' ?>">
            <i class='bx bx-list-ul'></i> Tous les Bons (<?= $countTotal ?>)
        </a>

        <a href="<?= BASE_URL ?>/achats?payment_mode=cash<?= !empty($filterSearch) ? '&search='.urlencode($filterSearch) : '' ?><?= !empty($filterSupplierId) ? '&supplier_id='.$filterSupplierId : '' ?>" 
           ...
           style="padding: 5px 12px; font-size: 0.82rem; <?= ($filterPaymentMode !== 'cash') ? 'background: #F0FDF4; color: #16A34A; border: 1px solid #DCFCE7;' : '' ?>">
            🟢 Réglés Comptant (<?= $countCash ?>)
        </a>

        <a href="<?= BASE_URL ?>/achats?payment_mode=credit<?= !empty($filterSearch) ? '&search='.urlencode($filterSearch) : '' ?><?= !empty($filterSupplierId) ? '&supplier_id='.$filterSupplierId : '' ?>" 
           ...
           style="padding: 5px 12px; font-size: 0.82rem; <?= ($filterPaymentMode !== 'credit') ? 'background: #FFFBEB; color: #D97706; border: 1px solid #FEF3C7;' : '' ?>">
            🟠 À Crédit / Dettes (<?= $countCredit ?>)
        </a>
    </div>

    <!-- MULTI-CRITERIA FILTER FORM -->
    <form action="<?= BASE_URL ?>/achats" method="GET" style="display: flex; gap: 12px; align-items: flex-end; flex-wrap: wrap;">
        <input type="hidden" name="payment_mode" value="<?= htmlspecialchars($filterPaymentMode ?? 'all') ?>">

        <!-- SEARCH -->
        <div style="flex: 2; min-width: 180px;">
            <label class="form-label" style="font-size: 0.82rem; margin-bottom: 4px; color: #64748B;">Recherche par N° BL / Réf</label>
            <div style="position: relative;">
                <input type="text" name="search" class="form-control" placeholder="N° achat, BL fournisseur, notes..." value="<?= htmlspecialchars($filterSearch ?? '') ?>" style="padding-left: 32px; font-size: 0.88rem;">
        ...
        <div style="display: flex; gap: 8px;">
            ...
            <?php if (!empty($filterSearch) || !empty($filterSupplierId) || ($filterPaymentMode !== 'all' && !empty($filterPaymentMode)) || !empty($filterStartDate) || !empty($filterEndDate) || !empty($filterPeriod)): ?>
                <a href="<?= BASE_URL ?>/achats" class="btn btn-primary" style="padding: 8px 12px; font-size: 0.88rem; background: #64748B;" title="Réinitialiser les filtres">
    ...
    <div style="display: flex; gap: 8px; margin-top: 12px; padding-top: 10px; border-top: 1px solid #F1F5F9; align-items: center; flex-wrap: wrap;">
        ...
        <a href="<?= BASE_URL ?>/achats?period=today<?= !empty($filterPaymentMode) ? '&payment_mode='.$filterPaymentMode : '' ?>" class="btn <?= (($filterPeriod ?? '') === 'today') ? 'btn-accent' : 'btn-primary' ?>" style="padding: 3px 8px; font-size: 0.75rem; <?= (($filterPeriod ?? '') !== 'today') ? 'background: #F1F5F9; color: #334155;' : '' ?>">
            Aujourd'hui
        </a>
        <a href="<?= BASE_URL ?>/achats?period=week<?= !empty($filterPaymentMode) ? '&payment_mode='.$filterPaymentMode : '' ?>" class="btn <?= (($filterPeriod ?? '') === 'week') ? 'btn-accent' : 'btn-primary' ?>" style="padding: 3px 8px; font-size: 0.75rem; <?= (($filterPeriod ?? '') !== 'week') ? 'background: #F1F5F9; color: #334155;' : '' ?>">
            Cette Semaine
        </a>
        <a href="<?= BASE_URL ?>/achats?period=month<?= !empty($filterPaymentMode) ? '&payment_mode='.$filterPaymentMode : '' ?>" class="btn <?= (($filterPeriod ?? '') === 'month') ? 'btn-accent' : 'btn-primary' ?>" style="padding: 3px 8px; font-size: 0.75rem; <?= (($filterPeriod ?? '') !== 'month') ? 'background: #F1F5F9; color: #334155;' : '' ?>">
            Ce Mois
        </a>
        <a href="<?= BASE_URL ?>/achats" class="btn btn-primary" style="padding: 3px 8px; font-size: 0.75rem; background: #F1F5F9; color: #334155;">
            Tous les achats
        </a>
    </div>
</div>
...
<div class="card">
    <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            <span><i class='bx bx-list-ul'></i> Journal des Entrées de Stock & Bons d'Achat</span>
            <span style="font-size: 0.82rem; font-weight: 700; color: var(--c-navy); background: #F1F5F9; padding: 3px 8px; border-radius: 6px;">
                <?= count($achats) ?> bon(s) affiché(s)
            </span>
        </div>
        <div style="display: flex; align-items: center; gap: 15px; flex-wrap: wrap;">
            <span style="font-size: 0.85rem; color: #64748B;">
                Total Sélection : <strong style="color: var(--c-navy); font-size: 0.95rem;"><?= number_format($filteredTotal, 0, ',', ' ') ?> FCFA</strong>
            </span>
            <a href="<?= BASE_URL ?>/achats/form" class="btn btn-accent"><i class='bx bx-plus-circle'></i> Nouvel Approvisionnement</a>
        </div>
    </div>
    <div class="table-responsive">
        <table class="table">
            ...
            <tbody>
                <?php if (empty($achats)): ?>
                <tr><td colspan="8" style="text-align: center; padding: 30px; color: var(--c-gray-600);">Aucun approvisionnement enregistré.</td></tr>
                <?php else: ?>
                    <?php foreach ($achats as $a): ?>
                    <?php 
                        $isCredit = ($a['payment_method_id'] == 5); 
                        $isValid = ($a['status'] === 'Valid');
                    ?>
                    <tr style="<?= !$isValid ? 'opacity: 0.65; background: #FFF5F5;' : '' ?>">
                        <td>
                            <a href="<?= BASE_URL ?>/achats/order/<?= $a['id'] ?>" style="font-weight: 800; color: var(--c-navy); text-decoration: none;">
                                <?= htmlspecialchars($a['id']) ?>
                            </a>
                        </td>
                        <td><?= date('d/m/Y', strtotime($a['purchase_date'])) ?></td>
                        <td><strong><?= htmlspecialchars($a['supplier_name']) ?></strong></td>
                        <td>
                            <div style="font-weight: 600; color: var(--c-navy-dark); font-size: 0.9rem;">
                                <?= htmlspecialchars($a['products_summary']) ?>
                            </div>
                            <small style="color: #64748B;">
                                <?= intval($a['item_count']) ?> ligne(s) d'articles
                            </small>
                        </td>
                        <td style="font-weight: 800; font-size: 1.05rem; color: var(--c-navy);">
                            <?= number_format($a['total_amount'], 0, ',', ' ') ?> FCFA
                        </td>
                        <td>
                            <?php if ($isCredit): ?>
                            ...
                            <?php else: ?>
                                <span style="padding: 3px 8px; background: #D1FAE5; color: #065F46; border-radius: 10px; font-size: 0.78rem; font-weight: 700;">
                                    🟢 <?= htmlspecialchars($a['cash_account_name'] ?: 'Caisse') ?>
                                </span>
                            <?php endif; ?>
                        </td>
                        <td>
                        ...
                        <td style="text-align: center;">
                            <div style="display: flex; gap: 6px; justify-content: center;">
                                <a href="<?= BASE_URL ?>/achats/order/<?= $a['id'] ?>" class="btn btn-primary" style="padding: 4px 9px; font-size: 0.85rem;" title="Voir / Imprimer Bon de Réception">
                                    <i class='bx bx-printer'></i>
                                </a>
                                <?php if ($isValid && \App\Core\Helper::isAdmin()): ?>
                                    <a href="<?= BASE_URL ?>/achats/cancel/<?= $a['id'] ?>" class="btn btn-primary" style="padding: 4px 9px; font-size: 0.85rem; background-color: var(--c-danger);" onclick="return confirm('Annuler le bon d\'achat <?= $a['id'] ?> ?\n\nLe stock sera décrémenté et la caisse/dette sera rééquilibrée automatiquement.')" title="Annuler le bon (Admin)">
